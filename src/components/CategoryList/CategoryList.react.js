    ...
    if (this.highlight) {
      ...
      for (let i = 0; i < this.props.categories.length; i++) {
            ...
            if (query.has('filters')) {
              ...
              for (let i = 0; i < c.filters?.length; i++) {
                ...
                if (queryFilter === filter.filter) {
                  height += (i + 1) * (20 + 24); // added 24 for margin top & bottom
                  break;
                }
              }
            }
        ...
        if (id === 'classSeparator') {
          ...
          height = (height + 20 + 24) * (c.filters.length + 1);
        } else {
          height += 20 + 24;
        }
      }
    ...
    return (
      <div ref={this.listWrapperRef} className={`${styles.class_list} class_list`}>
        {this.props.categories.map(c => {
          ...
          const categoryActive = c.currentActive;
          const action = c.action;
          if (categoryActive) {
            return (
              <div key={id} ref="listWrapper" className={styles.sub_class_list}>
            ...
            <div key={id}>
              ...
              {this.state.openClasses.includes(id) &&
                c.filters.map((filterData, index) => {
                  ...
                  return (
                    <div className={styles.childLink} key={index}>
                      <Link
