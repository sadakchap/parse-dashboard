...
function AnalyticsQueryStore(state, action) {
  ...
  const urlPrefix = `${b4aSettings.BACK4APP_API_PATH}/${appsPath}/${action.app.slug}/explorer`;
  ...
  switch (action.type) {
    ...
    case ActionTypes.FETCH:
      return post(urlPrefix, queryToPayload(action.query)).then((result) => {
        ...
        return state.setIn(['queries', action.query.localId], {
          ...action.query,
          result,
        });
      });
    case ActionTypes.CREATE:
      return post(`${urlPrefix}/saveQuery`, queryToPayload(action.query)).then(() => {
        return state.setIn(['queries', action.query.objectId], {
          ...action.query,
        });
      });
