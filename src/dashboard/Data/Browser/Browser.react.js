...
class Browser extends DashboardView {
  ...
  getFooterMenuButtons() {
    return this.state.renderFooterMenu || this.state.showTour
      ? [
          <a key={0} onClick={() => this.setState({ showTour: true })}>
            Play intro
          </a>,
        ]
      : null;
  ...
  getTourConfig() {
    ...
    const steps = [
      ...
      },
      {
        ...
        element: () =>
          document.querySelector('[class^=class_list] [title="B4aVehicle"]') ||
          document.querySelector('[class^=class_list]'),
      ...
      },
      {
        ...
        element: () =>
          document.querySelector('[class^="section_contents"] [class^=subitem] a[class^=action]'),
    ...
    return {
      steps,
      onBeforeStart: () => {
        document.querySelector('[class^="section_contents"] > div > div').style.backgroundColor =
          '#0e69a0';
      ...
      onBeforeChange: function (targetElement) {
        ...
        if (
          !this._forcedStep &&
          typeof back4AppNavigation === 'object' &&
          typeof back4AppNavigation.onDatabaseBrowserTourStep === 'function'
        ) {
        ...
        switch (this._currentStep) {
          ...
          case 3:
            {
              if (!getCustomVehicleClassLink() && !unexpectedErrorThrown) {
                schema
                  .dispatch(ActionTypes.CREATE_CLASS, {
                  ...
                  .then(() => {
                    return context.currentApp.apiRequest(
                      'POST',
                      '/classes/B4aVehicle',
                      { name: 'Corolla', price: 19499, color: 'black' },
                      { useMasterKey: true }
                    );
                  })
                  .then(() => {
          ...
          case 4:
            if (!unexpectedErrorThrown) {
              this._introItems[4].element = document.querySelector(
                '[class^=browser] [class^=tableRow] > :nth-child(2) span'
              );
            }
            break;
          case 5:
        ...
        switch (this._currentStep) {
          case 0:
            if (this._introItems.length === 1) {
              ...
              document
                .querySelector('.introjs-button.introjs-prevbutton')
                .classList.add('introjs-disabled');
              targetElement.style.backgroundColor = 'inherit';
            }
            break;
          case 3:
            if (!unexpectedErrorThrown) {
              if (
                !document.querySelector('[class^=browser] [class^=tableRow] > :nth-child(2) span')
              ) {
  ...
  redirectToFirstClass(classList, context) {
    if (!classList.isEmpty()) {
      ...
      let classes = classList.filter(
        className => className !== '_Role' && className !== '_User' && className !== '_Installation'
      );
      classes.sort((a, b) => {
        if (a[0] === '_' && b[0] !== '_') {
          return -1;
  ...
  createClass(className, isProtected, shouldContinue = false) {
    ...
    this.props.schema
      .dispatch(ActionTypes.CREATE_CLASS, { className, clp })
      .then(() => {
  ...
  importClass(className, file) {
    return this.context.currentApp.importData(className, file).then(
      res => {
        return res;
      },
      error => {
  ...
  importRelation(className, relationName, file) {
    return this.context.currentApp.importRelationData(className, relationName, file).then(
      res => {
        return res;
      },
      error => {
  ...
  removeColumn(name, selectedColumn = false) {
    ...
    this.props.schema
      ...
      .catch(error => {
        ...
        this.showNote(errorDeletingNote, true);
        if (selectedColumn) this.setState({ columnToDelete: null });
        else this.setState({ showRemoveColumnDialog: false });
      })
      .finally(() => {
  ...
  handlePointerCmdClick({ className, id, field = 'objectId' }) {
    ...
    window.open(
      this.context.generatePath(`browser/${className}?filters=${encodeURIComponent(filters)}`),
      '_blank'
    );
  }
  ...
  renderContent() {
    ...
    if (this.state.showCreateClassDialog) {
      ...
      let parseServerSupportsPointerPermissions = this.context.currentApp.serverInfo.features
        .schemas.editClassLevelPermissions;
      let currentColumns = this.getClassColumns(className);
      const userPointers = [];
      const schemaSimplifiedData = {};
      const classSchema = this.props.schema.data.get('classes').get(this.props.params.className);
      if (classSchema) {
      ...
      extras = (
        <PermissionsDialog
          ...
          validateEntry={entry =>
            validateEntry(userPointers, entry, parseServerSupportsPointerPermissions)
          }
          onCancel={() => this.setState({ openSecurityDialog: false })}
          parseVersion={this.context.currentApp.serverInfo}
          onConfirm={perms =>
            this.handleCLPChange(perms).then(() => this.setState({ openSecurityDialog: false }))
          }
        />
      );
