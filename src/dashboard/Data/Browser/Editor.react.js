...
const Editor = ({ top, left, type, targetClass, value, readonly, width, onCommit, onCancel }) => {
  ...
  if (type === 'String') {
    ...
    if (readonly) {
      ...
      content = <DateTimeEditor value={value || new Date()} width={width} onCommit={onCommit} setFocus={true} />;
    }
    ...
    content = <FileEditor value={value} width={width} onCommit={onCommit} onCancel={onCancel} setFocus={setFocus} />;
  } else if (type === 'ACL') {
    ...
    content = <StringEditor value={value ? value.id : ''} width={width} onCommit={encodeCommit} setFocus={true} />;
  }
